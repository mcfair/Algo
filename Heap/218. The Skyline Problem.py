...
Use a list axis to record the positions of l and r. Use a list heights to record the height of the right-hand side 
of the position in axis with the same index. 
For example, heights[i] records the height between axis[i] and axis[i + 1], that is, the height on interval 
[axis[i], axis[i + 1]).
...
that is heights[idl], should be h after updated. 
Meanwhile the height of the right-hand side of r wouldn't change after updated. 
...
hence we don't need to record l. 
If else idl == idr, it implies there's a recorded building with height h whose right edge is larger than r, 
then we don't renew anything. 
