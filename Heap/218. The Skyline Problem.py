...
Use a list axis to record the positions of l and r. Use a list heights to record the height of the right-hand side of the position in axis with the same index. 
For example, heights[i] records the height between axis[i] and axis[i + 1], that is, the height on interval [axis[i], axis[i + 1]).
...
Since the current building is higher than or equal to anyone before it, the height of the right-hand side of l, 
that is heights[idl], should be h after updated. Meanwhile the height of the right-hand side of r wouldn't change after updated. 
That is, heights[idr] should be current heights[idr - 1] after updated.

If h != heights[idl - 1], renew axis[idl: idr] = [l, r] and heights[idl: idr] = [h, heights[idr - 1]].
Otherwise h == heights[idl - 1], it means there's a recorded building with height h whose left edge is less than l, 
hence we don't need to record l. If else idl == idr, it implies there's a recorded building with height h whose right edge is larger than r, then we don't renew anything. 
Otherwise we only record r and renew axis[idl: idr] = [r] and heights[idl: idr] = [heights[idr - 1]].
